...
class OrderCommandControllerTest {
    ...
    @Test
    public void 더미_주문_완료() throws Exception {
        Order order = Order.of(ANY_ORDER_ID, TEST_USER_ID, ANY_COMPLETED_AT);
        given(completeOrderUseCase.complete(any())).willReturn(order);

        mockMvc.perform(patch(ORDERS_ENDPOINT + "/{orderId}/completion", ANY_ORDER_ID)
        ...
        then(completeOrderUseCase).should()
                .complete(any());
    }
}